...
class FrontPageSource(menus.PageSource):
    ...
    def format_page(self, menu: HelpMenu, page):
        embed = discord.Embed(title="HayBot's Help", colour=0x0287C3)
        embed.description = inspect.cleandoc(
            f"""
            Run "{menu.ctx.clean_prefix}help command" for more info on a command.
            Run "{menu.ctx.clean_prefix}help category" for more info on a category.
        """
        )
        if self.index == 0:
            embed.add_field(
                name="HayBot's main features",
                value=("insert-features"),
                inline=False,
...
class GroupHelpPageSource(menus.ListPageSource):
    ...
    async def format_page(self, menu, commands):
        embed = discord.Embed(title=self.title, description=self.description, colour=discord.Colour(0x0287C3))
...
class PaginatedHelpCommand(commands.HelpCommand):
    ...
    async def send_command_help(self, command):
        ...
        embed = discord.Embed(colour=discord.Colour(0x0287C3))
